...
- **Permission bubble** (`permission.rs` + the dedicated `permission` window) — when Claude requests a tool permission, decide **Allow/Deny from a floating bubble** (anchored over the pet) or global **Ctrl+Shift+Y/N**, instead of the terminal. The blocking `burnrat permission` command hook holds the request on the bridge until you decide; on timeout/dismissal it **defers to Claude's own prompt** (never silently allows/blocks). [clawd #2]
